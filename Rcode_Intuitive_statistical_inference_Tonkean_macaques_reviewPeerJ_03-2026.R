...
NbSuccess.Cond.1 <- NbSuccess.Cond.Ind %>% filter(Test_condition=="cond1")
wilcox.test(NbSuccess.Cond.1$Perf, alternative='two.sided', mu=0.5, paired=FALSE)
...
# Creating variable "Reward obtained in previous trial"
data$Reward_Obtained_PrevTrial <- "nothing"

data <- data %>%
  arrange(ID_individual, Test_condition, Session) %>%
  group_by(ID_individual, Test_condition, Session) %>%
  mutate(
    Reward_Obtained_PrevTrial = lag(Selected_reward),
    Success_lag = lag(Success)
  ) %>%
  ungroup()

data$Reward_Obtained_PrevTrial <- fct_collapse(data$Reward_Obtained_PrevTrial, "Red_Bean" = c("Red bean", "Red Bean"))
data$Reward_Obtained_PrevTrial <- fct_relevel(data$Reward_Obtained_PrevTrial, c("Red_Bean", "Peanut"))
levels(data$Reward_Obtained_PrevTrial)
data$Strategy_stay <- NA

# Creating variable "Strategy stay or shift"
for (i in 1:nrow(data)){
  if (!is.na(data$Success_lag[i])){
    if (data$Success_lag[i]==data$Success[i]){
      data$Strategy_stay[i] <- 1
    } else {data$Strategy_stay[i] <- 0 }
  }
}
data$Strategy_stay <- as.factor(data$Strategy_stay)

#Model Test Win_Stay-Lose_Shift strategy - per individuals - all conditions
Individual <- c("Abricot","Alaryc","Barnabe","Eric","Ficelle","Horus","Nema","Nereis","Olli","Walt")

for (i in 1:length(Individual)){
  IndGLM <- data %>% filter(ID_individual==Individual[i])
  model_WSLS <- glmer(Strategy_stay ~ Reward_Obtained_PrevTrial + (1|Session), data = IndGLM, family = binomial(link = "logit"), control = glmerControl(optimizer = "bobyqa"))
  coeff <- summary(model_WSLS)$coefficients
  coeff_df <- as.data.frame(coeff)
  file_name <- paste0("WSLS_", Individual[i], ".csv")
...
#### Performance at first trial ####
First_table <- data %>%group_by(Test_condition, Trial) %>%
  filter(Trial=="1")%>%
  dplyr::summarise(NbSucces=sum(Success),N=n()) %>%
  dplyr::mutate(Perf= NbSucces/N)

First_table$Binom_p_value <- "initialisation"

for (i in 1:nrow(First_table)){
  n <- First_table$NbSucces[i]
  Tot <- First_table$N[i]
  Test_Binom2sided <- binom.test(n,Tot,p=0.5,alternative="two.sided")
  First_table$Binom_p_value[i] <- Test_Binom2sided[["p.value"]]
}
write.table(First_table,"First_trial_table.csv",sep=";",row.names=FALSE)

##### Video coding reliability #####
####################################

library(irr) # version 0.84.1
video_coding <- read.csv2(file = "Kappa_data.csv", header = TRUE, sep = ';', dec = ',', stringsAsFactors = TRUE)
str(video_coding)
kappa2(video_coding)
